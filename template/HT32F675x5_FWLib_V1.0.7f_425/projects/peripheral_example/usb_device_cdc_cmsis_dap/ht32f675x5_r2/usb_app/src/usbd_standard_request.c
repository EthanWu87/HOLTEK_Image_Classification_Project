...
static stUsbBuffer_t m_stHIDReportDescriptor = {.pu8Buf = NULL, .u16BufLen = 0};
...
void usb_device_standard_descriptor_init(void)
{
    ...
    // 5. HID Report Descriptor
    m_stHIDReportDescriptor.pu8Buf    = (uint8_t *)g_au8HidDataReportDescriptor;
    m_stHIDReportDescriptor.u16BufLen = sizeof(g_au8HidDataReportDescriptor);
}
...
/**
 * @brief  Read efuse product information.
 */
static void usb_device_read_efuse_product_date(char *pData)
{
    uint8_t u8efuse[6];
    uint8_t u8MAC[6];
    uint8_t i = 0, j = 0;
    ...
    u8MAC[0] = u8efuse[5];                                                        // Host and Socket ID
    u8MAC[1] = (LO_UINT8(u8efuse[0]) << HALF_BYTE_BITS) | (HI_UINT8(u8efuse[0])); // Year and Month
    u8MAC[2] = u8efuse[1];                                                        // Day
    u8MAC[3] = u8efuse[2];                                                        // Hour
    u8MAC[4] = u8efuse[3] & EFUSE_PRODUCTION_TIME_MINUTE_MSK;                     // Minute
    u8MAC[5] = u8efuse[4] & EFUSE_PRODUCTION_TIME_SECOND_MSK;                     // Second

    for (i = 0; i < sizeof(u8MAC); i++)
    {
        pData[j++] = hexEncode[(u8MAC[i] >> 4) & 0xf];
        pData[j++] = hexEncode[(u8MAC[i]) & 0xf];
    }
    pData[j] = 0;
}
...
static void usb_device_get_descriptor(uint8_t *pu8Setup)
{
    ...
    switch (pu8Setup[3])
    {
        ...
        case USB_DT_STRING:
        {
            ...
            else if (pu8Setup[2] == 3)
            {
                char serialNum[13];
                usb_device_read_efuse_product_date(serialNum);
                usb_device_get_string((uint8_t *)serialNum, m_au8StringDescriptorBuf, &stUsbBuf.u16BufLen);
                stUsbBuf.pu8Buf = m_au8StringDescriptorBuf;
            }
            else if (pu8Setup[2] == 4)
            {
                usb_device_get_string((uint8_t *)USB_STRING_DESCRIPTOR_PRODUCT1, m_au8StringDescriptorBuf,
                                      &stUsbBuf.u16BufLen);
                stUsbBuf.pu8Buf = m_au8StringDescriptorBuf;
            }
            else if (pu8Setup[2] == 5)
            {
                usb_device_get_string((uint8_t *)USB_STRING_DESCRIPTOR_PRODUCT2, m_au8StringDescriptorBuf,
                                      &stUsbBuf.u16BufLen);
                stUsbBuf.pu8Buf = m_au8StringDescriptorBuf;
            }
            break;
        ...
        {
            stUsbBuf.pu8Buf    = m_stHIDReportDescriptor.pu8Buf;
            stUsbBuf.u16BufLen = m_stHIDReportDescriptor.u16BufLen;
            break;
        }
        default:
        {
