...
static uint8_t gu8UartRxBuf[GLS_UART_BUF_SIZE] = {0};
...
static EN_ERR_STA_T uart_dma_init(void)
{
    ...
    rom_hw_uart_set_rxfifo_thld(GLS_UART_HANDLE, 1);
    ...
    stDmaInit.unCfg.u32DMACfg = DMA_INIT_PERI_TO_MEM_DEFAULT_CFG(GLS_UART_BUF_SIZE, DMA_DATA_1BYTE);
...
EN_ERR_STA_T uart_init(uart_recv_callback_t recvCb)
{
    ...
    // 1. Init UART IO.
    rom_hw_gpio_set_pin_pid(GLS_UART_PORT_TX, GLS_UART_PIN_TX, PID_UART1_TXD);
    rom_hw_gpio_set_pin_pid(GLS_UART_PORT_RX, GLS_UART_PIN_RX, PID_UART1_RXD);
    rom_hw_gpio_set_pin_pull_mode(GLS_UART_PORT_TX, GLS_UART_PIN_TX, GPIO_PULL_UP);
    rom_hw_gpio_set_pin_pull_mode(GLS_UART_PORT_RX, GLS_UART_PIN_RX, GPIO_PULL_UP);
    ...
    // 3. UART Init.
    stUartInit.u32UartBaudRate = GLS_UART_BPS;
    stUartInit.unUartCfg.u32UartCfg = UART_INIT_DEFAULT(UART_PARITY_NONE);
    
    enRet = rom_hw_uart_init(GLS_UART_HANDLE, &stUartInit);
    if (ERR_STA_OK != enRet)
        return enRet;
...
{
    return rom_hw_uart_transmit(GLS_UART_HANDLE,pu8Buf,u16Len);
}
...
EN_ERR_STA_T uart_deinit(void)
{
    ...
    rom_hw_gpio_set_pin_input_output(GLS_UART_PORT_TX, GLS_UART_PIN_TX, GPIO_MODE_IMPEDANCE);
    rom_hw_gpio_set_pin_input_output(GLS_UART_PORT_RX, GLS_UART_PIN_RX, GPIO_MODE_IMPEDANCE);
