...
{
    // Information
    uint16_t u16ProjectID;
    uint8_t u8ChipType;      // Chip type. 0x04: HT32F675x5
    uint8_t u8UpgradeDst;
    uint8_t u8RomVer;        // Rom version.
    uint8_t u8Boot2Ver;      // Secondary boot version.
    uint16_t u16AppCodeVer;  // App code version.
    uint32_t u32BinCodeSize; // Bin code size
    uint32_t u32BinCodeCrc;
} stBleOtaInfo_t;

const stBleOtaInfo_t gstBleOtaInfo __attribute__((section(".fw.bleota.file.info"))) =
{
    .u16ProjectID     = HT32_BLE_SOC_FLAG,
    .u8ChipType       = HT32F675x5,
    .u8RomVer         = ROM_VERSION,
    .u8Boot2Ver       = BOOT2_VERSION,
    .u16AppCodeVer    = APP_VERSION,
    .u32BinCodeSize   = 0,
    .u32BinCodeCrc    = 0
};
...
static void system_power_init(EN_PMU_PWR_SEL_T enSel)
{
   ...
   {
		rom_hw_pmu_set_ldo_act_voltage(EN_LDO_ACT_1100mV);
      rom_hw_pmu_set_ldo_dig_voltage(EN_LDO_DIG_1000mV);
      rom_hw_pmu_set_ldo_ret_sleep_voltage(EN_LDO_RET_950mV);
	}
   else
   {
		rom_hw_pmu_dcdc_init();
      rom_hw_pmu_set_dcdc_act_voltage(EN_DCDC_ACT_VOLT_1100mV);
      rom_hw_pmu_set_ldo_dig_voltage(EN_LDO_DIG_1000mV);
      rom_hw_pmu_set_dcdc_ret_voltage(EN_DCDC_LPWR_VOLT_950mV);
	...
	if (enSel == PWR_SEL_LDO)
   {
		rom_hw_pmu_set_ldo_act_voltage(EN_LDO_ACT_1150mV);
      rom_hw_pmu_set_ldo_dig_voltage(EN_LDO_DIG_1050mV);
      rom_hw_pmu_set_ldo_ret_sleep_voltage(EN_LDO_RET_950mV);
	}
   else
   {
		rom_hw_pmu_dcdc_init();
      rom_hw_pmu_set_dcdc_act_voltage(EN_DCDC_ACT_VOLT_1150mV);
      rom_hw_pmu_set_ldo_dig_voltage(EN_LDO_DIG_1050mV);
      rom_hw_pmu_set_dcdc_ret_voltage(EN_DCDC_LPWR_VOLT_950mV);
      rom_hw_pmu_set_dcdc_ret_clk_divisor(EN_DCDC_LPWR_CLK_1K);
      rom_hw_pmu_enable_dcdc_ret_output();
...
static void system_clock_init(EN_SYS_CLK_SRC_SEL_T enSysClkSrc,EN_LPWR_CLK_SRC_T enLpwrClkSrc)
{
   ...
   /* System clock */
   rom_hw_pmu_sel_dcxo_hclk_pwr(EN_DCXO_HCLK_PWR_DCDC_ACT);
   rom_hal_pmu_cfg_dcxo_hclk_param(DCXO_HCLK_IB_5, DCXO_HCLK_NGM_3, DCXO_HCLK_CAP_18PF);
   rom_hal_pmu_set_sys_clk_src(enSysClkSrc, DCXO_HCLK_STABLE_TIME);

	/* Lower Power Clock */
	rom_hw_crg_enable_clk_gate(CRG_RTC_APB_CLK_GATE);
	rom_hw_crg_enable_clk_gate(CRG_RTC_RC_LCLK_GATE);
   rom_hal_pmu_cfg_dcxo_lclk_param(DCXO_LCLK_BUF_3,DCXO_LCLK_IB_3,DCXO_LCLK_NGM_3,DCXO_LCLK_CAP_7PF4);
   rom_hal_pmu_set_lpwr_clk_src(enLpwrClkSrc,DCXO_LCLK_STABLE_TIME_500MS);
}
...
int main(void)
{
   ...
   system_clock_init(EN_SYS_CLK_DCXO16M,LPWR_CLK_DCXO_LCLK);
   peripheral_init();
