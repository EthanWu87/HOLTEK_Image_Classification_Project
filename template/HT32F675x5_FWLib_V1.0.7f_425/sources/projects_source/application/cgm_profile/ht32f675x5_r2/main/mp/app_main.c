...
static void system_clock_init(EN_SYS_CLK_SRC_SEL_T enSysClkSrc,EN_LPWR_CLK_SRC_T enLpwrClkSrc)
{
    ...
    rom_hal_pmu_cfg_dcxo_lclk_param(DCXO_LCLK_BUF_3,DCXO_LCLK_IB_3,DCXO_LCLK_NGM_3,DCXO_LCLK_CAP_10PF);
...
static void peripheral_init(void)
{
    ...
    /* sw irq for cp wakeup mp*/
    NVIC_ClearPendingIRQ(SW_IRQ2);
    NVIC_SetPriority(SW_IRQ2,3);
    NVIC_EnableIRQ(SW_IRQ2);
    rom_hw_sys_ctrl_enable_peri_int(SYS_CTRL_MP,SW_IRQ2);
...
/**
 * @brief  SW IRQ Handler.
 */
void SW_IRQ2_Handler(void)
{
    uint8_t intFlag;

    // cp wakeup mp by sw irq2 
