...
uint8_t  m_u8BlePeerMac[6];
static uint8_t  u8scanReportBuf[256];

uint16_t u16MasterConnectHandle = 0;
uint16_t u16SlaveConnectHandle = 0;
...
static void ble_event_adv_report(uint8_t *pu8Buf, uint32_t u32BufSize)
{
    ...
    for (u8NextReportOffset = 2; u8NextReportOffset < u32BufSize; u8NextReportOffset += u16ReportSize)
    {
        ...
        // 4.5 RSSI
        int8_t s8Rssi = (int8_t)(pu8Buf[u8NextReportOffset + 10 + u8AdvDataLen]);
...
//static uint8_t test = 0; // GNU Warning Issue
/**
...
static void ble_event_handler(uint32_t u32Cmd, const void *pData, uint32_t u32Len)
{
    ...
    switch (u32Cmd)
    {
        ...
        case MSG_BLE_CONNECTED_IND:
        {
            ...
            //Discover the services of slave
            uint8_t PeerAddr[] = BLE_SCAN_PEER_MAC;
            if(0 == memcmp(PeerAddr,pstEvent->Peer_Address,sizeof(PeerAddr)))
            {
                u16MasterConnectHandle = pstEvent->Connection_Handle;
                rom_gatt_client_api_discover_primary_services(pstEvent->Connection_Handle);
            }
            else
            {
                u16SlaveConnectHandle = pstEvent->Connection_Handle;
            }
            break;
        }
        case MSG_BLE_DISCONNECTED_IND:
        {
            ...
            MLOG88D(0xa005, "MSG_BLE_DISCONNECTED_IND Handle = 0x%X, reason = 0x%X",
