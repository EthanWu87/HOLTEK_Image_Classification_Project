...
uint8_t edge_impulse_clean_buffer[(HM01B0_IMAGE_WIDTH - 4) * HM01B0_IMAGE_HEIGHT];
  ...
  while(1)
  {
    if(g_frame_ready)
    {
      ...
      for(int r = 0; r < 120; r++)
      {
        ...
        for(int c = 0; c < 160; c++) 
        {
          edge_impulse_clean_buffer[pixel_idx++] = line_ptr[c];
        }
      }

      // 精準傳送 160x120 = 19200 位元組給 Python
      for(uint32_t i = 0; i < (HM01B0_IMAGE_WIDTH - 4) * HM01B0_IMAGE_HEIGHT; i++)
      {
          while(usart_flag_get(USART1, USART_TDBE_FLAG) == RESET);
