...
void hm01b0_init(i2c_handle_type* hi2c)
{
  ...
  uint8_t bit_control_val         = 0x22;   /* 1-bit mode, Gated PCLK (Bit 5) */   
...
void hm01b0_spi_dma_init(void* buffer, uint32_t size)
{
  ...
  gpio_init_struct.gpio_pull      = GPIO_PULL_DOWN; /* Ensure stable PCLK when gated */
  
  gpio_init_struct.gpio_pins = HM01B0_PCLK_PIN | HM01B0_D0_PIN;
  gpio_init(HM01B0_PCLK_PORT, &gpio_init_struct);

  /* HREF (PA4) 設為 Input 以進行 EXINT 模擬硬體觸發 */
  ...
  /* 3. SPI Configuration (Slave Mode) */
  ...
  /* 5. VSYNC/HREF EXINT Configuration */
  /* VSYNC (PB0) */
  ...
  exint_init_struct.line_polarity = EXINT_TRIGGER_RISING_EDGE; /* Only on Start */
  exint_init(&exint_init_struct);

  /* HREF (PA4) - 用於反轉硬體觸發訊號 */
  gpio_exint_line_config(GPIO_PORT_SOURCE_GPIOA, GPIO_PINS_SOURCE4);
  exint_init_struct.line_select = EXINT_LINE_4;
  exint_init_struct.line_polarity = EXINT_TRIGGER_BOTH_EDGE; /* 偵測 Rising 與 Falling */
  exint_init(&exint_init_struct);
